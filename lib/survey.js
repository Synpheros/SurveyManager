...
class Survey{
	...
	save(callback){
		...
		if(!this._id){ // insert new survey 
			...
			if(this.teacher) attrs["teacher"] = this.teacher;
			...
			aux.load(function(err,result){
				...
				if(s.teacher && s.teacher !== aux.teacher) 				attrs['teacher'] = s.teacher;
	...
	delClassroom(classroom, callback){
		var s = this;
		...
		async.waterfall([
			controller.online,
			controller.auth,
			controller.delParticipants(classroom,s)
		], function (err, result) {
			if(!err){
				for(var i = 0; i < s.classrooms.length; i++){
					console.log(s.classrooms[i]._id.toString() + " " + classroom._id.toString())
					if(s.classrooms[i]._id.toString() == classroom._id.toString()){
						console.log("encontrado");
						s.classrooms = s.classrooms.splice(i+1,1);
						break;
					}
				}
			}
	...
	delete(callback){
		...
		if(this.teacher)
			toRemove.push(controller.remove(this.teacher))
 ...
 * @param list 
 */
function getClassResponses(survey, classroom, list){
	return function(callback){
		var s = this;
		async.waterfall([
			controller.auth,
			controller.getClassResponses(survey, classroom, list),
		], function (err, result) {
			callback(null);
		});
	}
